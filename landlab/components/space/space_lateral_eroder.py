...
from .ext.calc_sequential_ero_depo_lateral import _sequential_ero_depo_lateral
...
class SpaceLateralEroder(Component):
    ...
    ...     PriorityFloodFlowRouter, SpaceLateralEroder
    ... )
    ...
    **Additional References**
    ...
    def __init__(
        ...
        """Initialize the SpaceLateralEroder model.
        ...
        lateral_erosion: boolean
            Use lateral erosion algorithm (True) or use SPACE without lateral erosion (False)
        ...
        if grid.at_node["flow__receiver_node"].size != grid.size("node"):
            raise NotImplementedError(
                ...
                "verified that SpaceLateralEroder is compatible with "
        ...
        self._kw = kw
        self._F = F
        self._wid_coeff = wid_coeff  # coefficient for calculating channel width
        self._wid_exp = wid_exp  # exponent for calculating channel width

        if type(lateral_erosion) is not bool:
            raise ValueError("Value for 'lateral_erosion' should be a boolean")

        self._lateral_erosion = lateral_erosion
    ...
    def run_one_step_basic(self, dt=10):
        ...
        lat_nodes = np.zeros_like(link_to_rcvr)
        vol_lat_dt = np.zeros_like(H)
        ...
        K_bed_vector = np.broadcast_to(self._K_br, self._q.shape)
        ...
        runoffms = (self._Klr * self._F / self._kw) ** 2
        # Kl_bed is calculated from ratio of lateral to vertical K parameters
        Kl_bed = K_bed_vector * self._Klr
        Kl_sed = K_sed_vector * self._Klr
        wd = self._wid_coeff * (da * runoffms) ** self._wid_exp

        vol_SSY_riv = _sequential_ero_depo_lateral(
            stack_flip_ud_sel,
            r,
        ...
        if self._lateral_erosion:
            ...
            if self._UC == 1:
                voldiff = (z + wd - z[r]) * self.grid.dx**2

            # TB model: entire lat node must be eroded before lateral erosion
            # occurs
            if self._TB == 1:
                voldiff = (z[lat_nodes] - z[r]) * self.grid.dx**2
            ...
            # timestep in it
            condition_b = vol_lat[lat_nodes] >= voldiff
            action_locs = np.where(np.logical_and(condition_a, condition_b))
            erosion = z[lat_nodes[action_locs]] - z[r[action_locs]]

            # Account for porosity
            soilErosion = np.minimum(H[lat_nodes[action_locs]], erosion)
            H[lat_nodes[action_locs]] -= soilErosion
            br[lat_nodes[action_locs]] -= erosion - soilErosion
            vol_lat[lat_nodes[action_locs]] = 0.0
